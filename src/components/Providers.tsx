...
export const Providers: React.FC = () => {
  ...
  const handleDownloadSampleCSV = () => {
    const csvContent = [
      ...
      'John,Smith,1234567890,Family Medicine,active,(555) 123-4567,john.smith@example.com',
      'Jane,Doe,0987654321,Internal Medicine,active,(555) 987-6543,jane.doe@example.com'
    ].join('\n');
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = 'providers-sample.csv';
    a.click();
    URL.revokeObjectURL(url);
  };
  ...
  const handleImportProviders = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const importedProviders: Partial<Provider>[] = [];
        ...
        for (let i = 1; i < lines.length; i++) {
          if (lines[i].trim()) {
            ...
            const providerData: Partial<Provider> = {
              ...
              homePhone: values[headers.indexOf('phone')] || values[headers.indexOf('home phone')] || '',
        ...
        for (const providerData of importedProviders) {
          try {
            const insertData: any = {
              ...
              home_phone: providerData.homePhone || null,
              cell_phone: providerData.cellPhone || null,
              fax_number: providerData.faxNumber || null,
        ...
        toast({
          ...
          description: `${successCount} providers imported successfully${errorCount > 0 ? `, ${errorCount} failed` : ''}.`,
        });
      } catch (error: any) {
        toast({
          title: "Import Failed",
